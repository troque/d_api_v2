...
class VigenciaController extends Controller
{
    ...
    public function getVigencias($estado)
    {

        $query = $this->repository->customQuery(
            function ($model) use ($estado) {
                return $model->where("estado", $estado)->orderBy("vigencia", "desc")->get();
            }
        );
    ...
    { {
            // error_log("insert");
            try {
                return VigenciaResource::make($this->repository->create($request->validated()["data"]["attributes"]));
            } catch (\Exception $e) {
    ...
    {
        return VigenciaResource::make($this->repository->update($request->validated()["data"]["attributes"], $id));
    }
