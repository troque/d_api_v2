...
class ActivarFuncionalidadesController extends Controller
{
    ...
    public function activarClasificacionTiposConducta($id_proceso_disciplinario)
    {
        ...
        $fase_evaluacion = count(DB::select("select uuid from evaluacion where id_proceso_disciplinario = '" . $id_proceso_disciplinario . "' and estado_evaluacion = 1 and estado = 2"));
    ...
    public function activarClasificacionTiposExpediente($id_proceso_disciplinario)
    {
        ...
        $id_etapa = $this->etapaActual($id_proceso_disciplinario);
        $fase_clasificacion = count(DB::select("select uuid from validar_clasificacion where id_proceso_disciplinario = '" . $id_proceso_disciplinario . "'"));
    ...
    public function activarRegistroFaseEvaluacionEtapaEvaluacion($id_proceso_disciplinario)
    {
        ...
        $fase_evaluacion = count(DB::select("select uuid from evaluacion where id_proceso_disciplinario = '" . $id_proceso_disciplinario . "' and eliminado = 0"));
    ...
    public function activarSoporteRadicado($id_proceso_disciplinario)
    {
        ...
        $fase_datos_interesado = count(DB::select("select uuid from interesado where id_proceso_disciplinario = '" . $id_proceso_disciplinario . "'"));
    ...
    public function getTituloProceso($id_proceso_disciplinario)
    {
        ...
        if ($respuesta != null) {
            $nombre = $respuesta[0]->nombre . " No. " . $respuesta[0]->radicado . " - " . $respuesta[0]->vigencia;
     ...
     * @return json
     */
    public function ValidarAgregrarClasificacion($id_proceso_disciplinario)
    {

        // SI LA ETAPA DEL PROCESO DISCIPLINARIO ES MAYOR A 1
        $query = DB::select("select id_etapa from proceso_disciplinario where uuid = '" . $id_proceso_disciplinario . "'");

        if (!empty($query) && $query[0]->id_etapa > 1) {
            $reciboDatos['agregar_clasificacion'] = false;
            $json['data']['attributes'] = $reciboDatos;
            return json_encode($json);
        }

        // SI EL TIPO DE EXPEDIENTE ES QUEJA INTERNA
        $query = DB::select("select id_tipo_expediente, id_tipo_queja from clasificacion_radicado where id_proceso_disciplinario = '" . $id_proceso_disciplinario . "'");

        if (count($query) > 0 && $query[0]->id_tipo_expediente ==  Constants::TIPO_EXPEDIENTE['queja'] &&  $query[0]->id_tipo_queja == Constants::TIPO_QUEJA['interna']) {
            $reciboDatos['agregar_clasificacion'] = false;
            $json['data']['attributes'] = $reciboDatos;
            return json_encode($json);
        }

        // SI EL TIPO DE EXPEDIENTE ES PROCESO DISCIPLINARIO
        $query = DB::select("select id_tipo_expediente, id_tipo_queja from clasificacion_radicado where id_proceso_disciplinario = '" . $id_proceso_disciplinario . "'");

        if (count($query) > 0 && $query[0]->id_tipo_expediente ==  Constants::TIPO_EXPEDIENTE['proceso_disciplinario']) {
            $reciboDatos['agregar_clasificacion'] = false;
            $json['data']['attributes'] = $reciboDatos;
            return json_encode($json);
        }

        // SI EL TIPO DE EXPEDIENTE ES PODER PREFERENTE
        $query = DB::select("select id_tipo_proceso from proceso_disciplinario where uuid = '" . $id_proceso_disciplinario . "' and id_tipo_proceso = " . Constants::TIPO_DE_PROCESO['poder_preferente']);
        ...
        // SI EL TIPO DE EXPEDIENTE ES DESGLOSE
        $query = DB::select("select id_tipo_proceso from proceso_disciplinario where uuid = '" . $id_proceso_disciplinario . "' and id_tipo_proceso = " . Constants::TIPO_DE_PROCESO['desglose']);
        ...
        $reciboDatos['agregar_clasificacion'] = true;
        $json['data']['attributes'] = $reciboDatos;
        return json_encode($json);
    }
}
