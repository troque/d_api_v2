...
class CierreEtapaController extends Controller
{
        ...
        try {
            ...
            if ($datosRequest['id_funcionario_asignado'] != null) {
                ...
                if (!empty($poder_preferente)) {
                    ...
                    if($queja_interna == false){
                        ProcesoDiciplinarioModel::where('uuid', $datosRequest['id_proceso_disciplinario'])->update(['id_etapa' => Constants::ETAPA['evaluacion_pd'], 'id_dependencia_actual' => $datosRequest['id_dependencia_actual']]);
                    }
                    else{
                        ProcesoDiciplinarioModel::where('uuid', $datosRequest['id_proceso_disciplinario'])
                        ->update(['usuario_comisionado' => auth()->user()->id, 'id_etapa' => Constants::ETAPA['investigacion_preliminar']]);
                    }
                }
    ...
    public function cierreEvaluacionArchivado($datosRequest)
    {
        ...
        ProcesoDiciplinarioModel::where('uuid', $datosRequest['id_proceso_disciplinario'])->update(['estado' => 2, 'id_dependencia_actual' => $datosRequest['id_dependencia_actual']]);
    ...
    public function cierreDeActuaciones(CierreEtapaFormRequest $request)
    {
        ...
        try {
            ...
            error_log($e);
            $error['estado'] = false;
            $error['error'] = 'LA ETAPA NO FUE CERRADA';
            return json_encode($error);
        }
    }
