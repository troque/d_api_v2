...
class BuscadorController extends Controller
{
    ...
    public function buscadorGeneral(BuscadorFormRequest $request)
    {
        ...
        $totalResults = $this->obtenerFilas($datosRequest);
        ...
        $query = "
            ...
            from (
            SELECT
                row_number() over(partition by pd.radicado, pd.vigencia order by 1) rn,
                ...
                UPPER(ent.nombre) AS entidad,
                UPPER(sec.nombre) AS sector,
                UPPER(mti.nombre) AS tipo_quejoso,
                UPPER(mtsp.nombre) AS sujeto_procesal,
                ...
                UPPER(mre.nombre) AS evaluacion,
                ev.resultado_evaluacion AS id_evaluacion,
                UPPER(mtc.nombre) AS tipo_de_conducta,
                act.auto,
                ma.nombre_actuacion,
            ...
            LEFT JOIN PROCESO_DESGLOSE pdd ON pdd.id_proceso_disciplinario = pd.uuid
            LEFT JOIN PROCESO_PODER_PREFERENTE pdpp ON pdpp.id_proceso_disciplinario = pd.uuid
            LEFT JOIN PROCESO_SINPROC pdsc ON pdsc.id_proceso_disciplinario = pd.uuid
            LEFT JOIN PROCESO_SIRIUS pdss ON pdss.id_proceso_disciplinario = pd.uuid
            ...
            LEFT JOIN antecedente a ON a.id_proceso_disciplinario = pd.uuid
            INNER JOIN mas_etapa me on me.id = pd.id_etapa
            INNER JOIN mas_dependencia_origen mdo on mdo.id = pd.id_dependencia            
            LEFT JOIN mas_dependencia_origen mdd ON mdd.id = pd.id_dependencia_duena
            LEFT JOIN users co ON co.id = pd.usuario_comisionado
            INNER JOIN mas_estado_proceso_disciplinario mepd on mepd.id = pd.estado
            LEFT JOIN users u ON u.name = lpd.id_funcionario_actual
            LEFT JOIN entidad_investigado ei ON ei.id_proceso_disciplinario = pd.uuid AND ei.estado = 1
            LEFT JOIN entidad ent ON ei.id_entidad = ent.identidad 
            LEFT JOIN sector sec ON ent.idsector = sec.idsector
            LEFT JOIN interesado i ON i.id_proceso_disciplinario = pd.uuid AND i.estado = 1
            LEFT JOIN mas_tipo_interesado mti on mti.id = i.id_tipo_interesao
            LEFT JOIN interesado tsp ON tsp.id_proceso_disciplinario = pd.uuid
            LEFT JOIN mas_tipo_sujeto_procesal mtsp on mtsp.id = tsp.id_tipo_sujeto_procesal
            LEFT JOIN evaluacion ev ON ev.id_proceso_disciplinario = pd.uuid AND (ev.estado = 2 or ev.estado = 3)
            LEFT JOIN mas_resultado_evaluacion mre on mre.id = ev.resultado_evaluacion
            LEFT JOIN tipo_conducta_proceso_disciplinario tcpd ON tcpd.id_proceso_disciplinario = pd.uuid
            LEFT JOIN mas_tipo_conducta mtc on mtc.id = tcpd.id_tipo_conducta            
            LEFT JOIN actuaciones act ON act.uuid_proceso_disciplinario = pd.uuid AND act.id_estado_actuacion = 5
            LEFT JOIN mas_actuaciones ma ON ma.id = act.id_actuacion
            WHERE pd.radicado LIKE '%" . $datosRequest["radicado"] . "%'
            ...
            AND (ent.identidad IS NULL OR ent.identidad LIKE '%" . $datosRequest["entidad"] . "%')
            AND (sec.idsector IS NULL OR sec.idsector LIKE '%" . $datosRequest["sector"] . "%')
            ...
            ORDER BY a.created_at DESC, lpd.created_at DESC, ei.created_at DESC, i.created_at DESC, tsp.created_at DESC, ev.created_at DESC, tcpd.created_at DESC, act.created_at DESC
            ) x
            where x.rn = 1
            ORDER BY x.radicado DESC, x.vigencia DESC            
            OFFSET ".($page == 1 ? '0' : ($perPage * ($page-1)))." ROWS
        ...
        for ($cont = 0; $cont < count($datos_filtrados); $cont++) {
            ...
            if (
                ...
                $reciboDatos['TotalPaginas'] = $totalPages;
                $reciboDatos['TotalRegistros'] = $totalResults;
    ...
    private function obtenerFilas($datosRequest){
       
        $query = "
            select
                count(x.radicado) as registros
            from (
            SELECT
                row_number() over(partition by pd.radicado, pd.vigencia order by 1) rn,
                pd.radicado AS radicado,
