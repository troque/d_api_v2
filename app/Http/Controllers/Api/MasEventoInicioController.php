...
class MasEventoInicioController extends Controller
{
    ...
    {
        try {
            return MasEventoInicioFormRequest::make($this->repository->create($request->validated()["data"]["attributes"]));
        } catch (\Exception $e) {
    ...
    {
        return MasEventoInicioResource::make($this->repository->update($request->validated()["data"]["attributes"], $id));
    }
}
