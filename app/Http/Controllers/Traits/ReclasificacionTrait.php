...
trait ReclasificacionTrait
{
    ...
    public static function reclasificacionPorTipoExpediente($id_proceso_disciplinario)
    {
        ...
        if (count($existe_actuaciones) > 0) {

            DB::table('PROCESO_DISCIPLINARIO')->where('UUID', $id_proceso_disciplinario)->update(['USUARIO_COMISIONADO' => NULL , 'ID_DEPENDENCIA_DUENA' => NULL]);

            DB::table('ACTUACION_POR_SEMAFORO')
            ->whereIn('ID_ACTUACION', function($query) use ($id_proceso_disciplinario) {
                $query->select('ID_ACTUACION')
                    ->from('ACTUACIONES')
                    ->where('UUID_PROCESO_DISCIPLINARIO', $id_proceso_disciplinario);
            })
            ->update(['ELIMINADO' => true]);
            
            DB::table('ARCHIVO_ACTUACIONES')
            ->whereIn('UUID_ACTUACION', function($query) use ($id_proceso_disciplinario) {
                $query->select('UUID')
                    ->from('ACTUACIONES')
                    ->where('UUID_PROCESO_DISCIPLINARIO', $id_proceso_disciplinario);
            })
        ...
        DB::table('log_proceso_disciplinario')->where('id_proceso_disciplinario', $id_proceso_disciplinario)->where('id_fase', Constants::FASE['lista_para_cierre_evaluacion'])->update(['eliminado' => true]);
