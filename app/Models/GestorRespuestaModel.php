...
class GestorRespuestaModel extends Model
{
    ...
    public function documentoSirius(){
        $documento = $this->hasOne(DocumentoSiriusModel::class, "uuid", "id_documento_sirius")->get();
        if($documento){            
            return DocumentoSiriusModel::where('id_log_proceso_disciplinario', $documento[0]->id_log_proceso_disciplinario)->orderBy('created_at')->get();
        }
        return null;
    }
