...
class AntecedenteController extends Controller
{
    ...
    public function store(AntecedenteFormRequest $request)
    {
        try {
            ...
            $datosRequest['descripcion'] = substr(strtoupper($datosRequest['descripcion']), 0, 4000);
            ...
            $logRequest['descripcion'] = substr("REGISTRO DE ANTECEDENTE: " .  strtoupper($datosRequest['descripcion']), 0, 4000);
    ...
    public function update(AntecedenteFormRequest $request, $id)
    {
        try {
            ...
            $logRequest['id_fase'] = Constants::FASE['antecedentes'];
            $logRequest['id_tipo_log'] = Constants::TIPO_LOG['fase'];
            $logRequest['descripcion'] = 'CAMBIO DE ESTADO ' . ($estadoInicial == '0' ? "INACTIVO" : "ACTIVO") . ' A ' . ($estadoFinal == '0' ? "INACTIVO" : "ACTIVO") . ' MOTIVO: ' . substr($datosRequest['estado_observacion'], 0, 3800);
            $logRequest['created_user'] = auth()->user()->name;
            $logRequest['id_estado'] = Constants::ESTADO_LOG_PROCESO_DISCIPLINARIO['remitido'];
