            ...
            if (count($etapa) > 0) {
                    ...
                    for ($cont_quejoso = 0; $cont_quejoso < $total_quejoso; $cont_quejoso++) {
                        ...
                        if (count($quejoso) > 0) {
                            ...
                            if (count($persona) > 0) {
                                ...
                                $datosInteresado['id_dependencia_entidad'] = $this->getDependenciaAmbientePruebas($proceso[0]->id_dependencia);
                                $datosInteresado['id_dependencia'] = $this->getDependenciaAmbientePruebas($proceso[0]->id_dependencia);
                                ...
                                //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                                $this->storeLogMigracion(Constants::ETAPA['captura_reparto'], $array_proceso->id, $this->getDependenciaAmbientePruebas($proceso[0]->id_dependencia), Constants::FASE['datos_interesado'], $array_interesado->id, auth()->user()->name);
                            }
                        }
                    }
                }
                ...
                if ($total_clasificacion > 0) {
                    ...
                    for ($cont_clasificacion = 0; $cont_clasificacion < $total_clasificacion; $cont_clasificacion++) {
                        ...
                        if ($datosClasificacion['id_etapa'] == 2) {
                            ...
                            //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                            $this->storeLogMigracion(Constants::ETAPA['evaluacion'], $array_proceso->id, $this->getDependenciaAmbientePruebas($proceso[0]->id_dependencia), Constants::FASE['validacion_clasificacion'], $array_validar_clasificacion->id, auth()->user()->name);
                ...
                if ($total_entidad > 0) {
                    ...
                    for ($cont_entidad = 0; $cont_entidad < $total_entidad; $cont_entidad++) {
                        ...
                        //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                        $this->storeLogMigracion($datosEntidad['id_etapa'], $array_proceso->id, $this->getDependenciaAmbientePruebas($proceso[0]->id_dependencia), Constants::FASE['entidad_investigado'], $array_entidad->id, auth()->user()->name);
                    }
                }
                ...
                //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                $this->storeLogMigracion($cerrar_etapa_captura_reparto[0]->id_etapa, $array_proceso->id, $this->getDependenciaAmbientePruebas($proceso[0]->id_dependencia), Constants::FASE['cierre_captura_reparto'], $array_cierre_cr->id, auth()->user()->name);
                ...
                if (count($evaluacion) > 0 && ($datosClasificacion['id_tipo_expediente'] != Constants::TIPO_EXPEDIENTE['derecho_peticion'] || $datosClasificacion['id_tipo_expediente'] != Constants::TIPO_EXPEDIENTE['tutela'])) {
                    ...
                    //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                    $this->storeLogMigracion(Constants::ETAPA['evaluacion'], $array_proceso->id, $this->getDependenciaAmbientePruebas($proceso[0]->id_dependencia), Constants::FASE['evaluacion'], $array_evaluacion->id, auth()->user()->name);
                }
                ...
                if (count($remision_queja) > 0 && ($datosClasificacion['id_tipo_expediente'] != Constants::TIPO_EXPEDIENTE['derecho_peticion'] || $datosClasificacion['id_tipo_expediente'] != Constants::TIPO_EXPEDIENTE['tutela'])) {
                    ...
                    $this->storeLogMigracion(Constants::ETAPA['evaluacion'], $array_proceso->id, $this->getDependenciaAmbientePruebas($proceso[0]->id_dependencia), Constants::FASE['remision_queja'], $array_remision_queja->id, auth()->user()->name);
                }
                ...
                if (count($cerrar_etapa_evaluacion) > 0) {
                    ...
                    // REGISTRA LA INFORMACIÓN EN EL LOG
                    $this->storeLogMigracion($id_etapa, $array_proceso->id, $dependencia_asignada, Constants::FASE['cierre_evaluacion'], $array_cierre_ev->id, auth()->user()->name);
                }
            }
        }
