...
class ActivarFuncionalidadesController extends Controller
{
    ...
    public function ValidarAgregrarClasificacion($id_proceso_disciplinario)
    {
        ...
        // SI EL TIPO DE EXPEDIENTE ES PODER PREFERENTE O DESGLOSE
        $query = DB::select("select id_tipo_expediente, id_tipo_queja from clasificacion_radicado where id_proceso_disciplinario = '" . $id_proceso_disciplinario . "'");

        $proceso_disciplinario = DB::select("select id_tipo_proceso from proceso_disciplinario
            where uuid = '" . $id_proceso_disciplinario . "'
            and (id_tipo_proceso = " . Constants::TIPO_DE_PROCESO['poder_preferente'] . " or id_tipo_proceso = " . Constants::TIPO_DE_PROCESO['desglose'] . ")");
