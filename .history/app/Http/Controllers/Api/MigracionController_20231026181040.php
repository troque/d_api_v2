...
class MigracionController extends Controller
{
    ...
    public function iniciarMigracionFromDisciplinarios()
    {
        ...
        for ($cont = 0; $cont < $total; $cont++) {
            ...
            if (count($etapa) > 0) {
                $_id_etapa = $etapa[0]->id_etapa;
                $_dependencia_actual = $etapa[0]->dependencia_finaliza;
                $_dependencia_finaliza = $etapa[0]->dependencia_finaliza;
            }

            if ($etapa == null || count($etapa) == 0) {
                $_id_etapa = 1;
                $_dependencia_actual = $proceso[0]->id_dependencia;
                $_dependencia_finaliza = "";
            }
            ...
            if (count($existe_proceso) == 0) {
                ...
                $datosProcesoDisciplinario['id_etapa'] = $_id_etapa;
                $datosProcesoDisciplinario['id_dependencia'] = $proceso[0]->id_dependencia;
                $datosProcesoDisciplinario['id_dependencia_actual'] = $_dependencia_actual;
                $datosProcesoDisciplinario['id_dependencia_duena'] = $_dependencia_finaliza;
                ...
                $this->storeLogMigracion($_id_etapa, $array_proceso->id, $proceso[0]->id_dependencia, Constants::FASE['inicio_proceso_disciplinario'], $array_proceso->id, auth()->user()->name, $datosProcesoDisciplinario['created_at']);
                ...
                if ($total_antecedentes > 0) {
                    ...
                    for ($cont_antecedente = 0; $cont_antecedente < $total_antecedentes; $cont_antecedente++) {
                        ...
                        if ($array_antecedentes != null) {
                            $this->storeLogMigracion($_id_etapa, $array_proceso->id, $antecedentes[$cont_antecedente]->id_dependencia, Constants::FASE['antecedentes'], $array_antecedentes->id, auth()->user()->name, $datosProcesoDisciplinario['created_at']);
                        }
                    }
                }
                    ...
                    for ($cont_quejoso = 0; $cont_quejoso < $total_quejoso; $cont_quejoso++) {
                        ...
                        if (count($quejoso) > 0) {
                            ...
                            if (count($persona) > 0) {
                                ...
                                //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                                $this->storeLogMigracion(Constants::ETAPA['captura_reparto'], $array_proceso->id, $proceso[0]->id_dependencia, Constants::FASE['datos_interesado'], $array_interesado->id, auth()->user()->name, $datosProcesoDisciplinario['created_at']);
                            }
                        }
                    }
                }
                ...
                if ($total_clasificacion > 0) {
                    ...
                    for ($cont_clasificacion = 0; $cont_clasificacion < $total_clasificacion; $cont_clasificacion++) {
                        ...
                        if ($datosClasificacion['id_etapa'] == 2) {
                            ...
                            //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                            $this->storeLogMigracion(Constants::ETAPA['evaluacion'], $array_proceso->id, $proceso[0]->id_dependencia, Constants::FASE['validacion_clasificacion'], $array_validar_clasificacion->id, auth()->user()->name, $datosProcesoDisciplinario['created_at']);
                ...
                if ($total_entidad > 0) {
                    ...
                    for ($cont_entidad = 0; $cont_entidad < $total_entidad; $cont_entidad++) {
                        ...
                        //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                        $this->storeLogMigracion($datosEntidad['id_etapa'], $array_proceso->id, $proceso[0]->id_dependencia, Constants::FASE['entidad_investigado'], $array_entidad->id, auth()->user()->name, $datosProcesoDisciplinario['created_at']);
                    }
                }
                ...
                //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                $this->storeLogMigracion($cerrar_etapa_captura_reparto[0]->id_etapa, $array_proceso->id, $proceso[0]->id_dependencia, Constants::FASE['cierre_captura_reparto'], $array_cierre_cr->id, auth()->user()->name, $datosProcesoDisciplinario['created_at']);
                ...
                if (count($evaluacion) > 0 && ($datosClasificacion['id_tipo_expediente'] != Constants::TIPO_EXPEDIENTE['derecho_peticion'] || $datosClasificacion['id_tipo_expediente'] != Constants::TIPO_EXPEDIENTE['tutela'])) {
                    ...
                    //REGISTRAR EN EL LOG PROCESO DISCIPLINARIO
                    $this->storeLogMigracion(Constants::ETAPA['evaluacion'], $array_proceso->id, $proceso[0]->id_dependencia, Constants::FASE['evaluacion'], $array_evaluacion->id, auth()->user()->name, $datosProcesoDisciplinario['created_at']);
                }
                ...
                if (count($remision_queja) > 0 && ($datosClasificacion['id_tipo_expediente'] != Constants::TIPO_EXPEDIENTE['derecho_peticion'] || $datosClasificacion['id_tipo_expediente'] != Constants::TIPO_EXPEDIENTE['tutela'])) {
                    ...
                    $requestRemisionQueja['id_dependencia_origen'] = $remision_queja[0]->registrado_en;
                    $requestRemisionQueja['id_dependencia_destino'] = $remision_queja[0]->asignado_dependencia;
                    ...
                    $this->storeLogMigracion(Constants::ETAPA['evaluacion'], $array_proceso->id, $proceso[0]->id_dependencia, Constants::FASE['remision_queja'], $array_remision_queja->id, auth()->user()->name, $datosProcesoDisciplinario['created_at']);
                }
