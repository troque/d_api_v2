...
class CierreEtapaController extends Controller
{
    ...
    public function store(CierreEtapaFormRequest $request)
    {
        ...
        $proceso_disciplinario =  DB::select("select id_dependencia_duena from proceso_disciplinario where uuid = '" . $datosRequest['id_proceso_disciplinario'] . "'");
        $datosRequest['id_dependencia_actual'] = $proceso_disciplinario[0]->id_dependencia_duena;
        ...
        try {
            ...
            if ($datosRequest['id_funcionario_asignado'] != null) {
                ...
                if (!empty($poder_preferente)) {
                    ProcesoDiciplinarioModel::where('uuid', $datosRequest['id_proceso_disciplinario'])->update(['id_etapa' => $poder_preferente[0]->id_etapa_asignada, 'id_dependencia_actual' => $datosRequest['id_dependencia_duena']]);
                } else {
                    ProcesoDiciplinarioModel::where('uuid', $datosRequest['id_proceso_disciplinario'])->update(['id_etapa' => Constants::ETAPA['evaluacion_pd'], 'id_dependencia_actual' => $datosRequest['id_dependencia_duena']]);
                }
    ...
    public function cierreEvaluacionArchivado($datosRequest)
    {
        ...
        ProcesoDiciplinarioModel::where('uuid', $datosRequest['id_proceso_disciplinario'])->update(['estado' => 2, 'id_dependencia_actual' => $datosRequest['id_dependencia_duena']]);
    ...
    public function cierreDeActuaciones(CierreEtapaFormRequest $request)
    {
        ...
        try {
            ...
            ProcesoDiciplinarioModel::where('uuid', $datosRequest['id_proceso_disciplinario'])->update(['estado' => Constants::ESTADO_LOG_PROCESO_DISCIPLINARIO['finalizado'], 'id_dependencia_actual' => $datosRequest['id_dependencia_actual']]);
