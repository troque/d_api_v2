...
class CierreEtapaController extends Controller
{
    ...
    public function cierreCapturaRepartoRepartoAleatorio($datosRequest)
    {
        ...
        try {
            ...
            $funcionario_asignado = $this->storeRepartoAleatorio(auth()->user()->name, $datosRequest['id_proceso_disciplinario'], auth()->user()->id_dependencia);

            if ($funcionario_asignado != null) {
                $funcionario_asignado_name = $funcionario_asignado->nombre_funcionario;
            } else {
                $error['estado'] = false;
                $error['error'] = 'No existen funcionario que tengan permisos para atender este tipo de expediente.';
                return json_encode($error);
            }
            ...
            // GUARDA POR DEFECTO VALIDAR CLASIFICACIÓN DEL RADICADO
