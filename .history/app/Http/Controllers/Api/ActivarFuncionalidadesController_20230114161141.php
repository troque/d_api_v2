...
class ActivarFuncionalidadesController extends Controller
{
    ...
    public function ValidarTipoProceso($id_proceso_disciplinario)
    {
        ...
        $proceso_disciplinario = DB::select("select id_tipo_proceso from proceso_disciplinario where id_proceso_disciplinario = '" . $id_proceso_disciplinario . "'
            and id_tipo_proceso = " . Constants::TIPO_DE_PROCESO['poder_preferente']);

        if (!empty($proceso_disciplinario) && ($proceso_disciplinario[0]->id_tipo_proceso == Constants::TIPO_DE_PROCESO['poder_preferente'] || $proceso_disciplinario[0]->id_tipo_proceso == Constants::TIPO_DE_PROCESO['poder_preferente'])) {
            $reciboDatos['poder_preferente'] = true;
        } else {
            $reciboDatos['poder_preferente'] = false;
        }
