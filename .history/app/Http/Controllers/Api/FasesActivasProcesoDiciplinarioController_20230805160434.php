...
class FasesActivasProcesoDiciplinarioController extends Controller
{
    ...
    public function validarEstadoFase($id_fase, $procesoDisciplinarioUUID)
    {
        ...
        if ($id_fase == Constants::FASE['cierre_captura_reparto']) {
            ...
            $fase_terminada = count(DB::select("select uuid from registro_seguimiento where id_proceso_disciplinario = '" . $procesoDisciplinarioUUID . "' and finalizado = 1"));

            if ($fase_terminada == 0) {
                $fase_en_proceso = count(DB::select("select uuid from registro_seguimiento where id_proceso_disciplinario = '" . $procesoDisciplinarioUUID . "' and finalizado = 0"));
            }
        }
