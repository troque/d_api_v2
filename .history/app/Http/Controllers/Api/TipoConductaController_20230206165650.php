...
class TipoConductaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = $query->select('mas_tipo_conducta.id', 'mas_tipo_conducta.nombre', 'mas_tipo_conducta.estado')->orderBy('mas_tipo_conducta.nombre', 'asc')->get();
    ...
    {
        try {
            return TipoConductaResource::make($this->repository->create($request->validated()["data"]["attributes"]));
        } catch (\Exception $e) {
    ...
    {
        return TipoConductaResource::make($this->repository->update($request->validated()["data"]["attributes"], $id));
    }
    ...
    public function getTiposConductasHabilitadas($id_proceso_disciplinario)
    {
        ...
        if (count($evaluacion) > 0) {
            $query = $query->select('mas_tipo_conducta.id', 'mas_tipo_conducta.nombre', 'mas_tipo_conducta.estado')->where('id', '<>', $evaluacion[0]->tipo_conducta)->orderBy('mas_tipo_conducta.nombre', 'asc')->get();
            return TipoConductaCollection::make($query);
        }
