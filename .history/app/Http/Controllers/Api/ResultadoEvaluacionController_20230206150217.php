...
class ResultadoEvaluacionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = $query->select('mas_resultado_evaluacion.id, mas_resultado_evaluacion.nombre, mas_resultado_evaluacion.estado')->where('estado', true)->orderBy('mas_resultado_evaluacion.nombre', 'asc')->get();
    ...
    {
        $query = ResultadoEvaluacionModel::query();
        $query = $query->select('mas_resultado_evaluacion.id, mas_resultado_evaluacion.nombre, mas_resultado_evaluacion.estado')->orderBy('mas_resultado_evaluacion.nombre', 'asc')->get();
        return ResultadoEvaluacionCollection::make($query);
    }
    ...
    public function getResultadoEvaluacionHabilitados($id_proceso_disciplinario)
    {
        ...
        $query = ResultadoEvaluacionModel::query();
        $evaluacion = EvaluacionModel::where("id_proceso_disciplinario", $id_proceso_disciplinario)->where('estado_evaluacion', Constants::ESTADOS['activo'])->where('eliminado', 0)->get();

        if (count($proceso) == 0 && count($evaluacion) > 0) {
            $query = $query->select('mas_resultado_evaluacion.id', 'mas_resultado_evaluacion.nombre', 'mas_resultado_evaluacion.estado')->where('id', '<>', $evaluacion[0]->resultado_evaluacion)->where('estado', Constants::ESTADOS['activo'])->orderBy('mas_resultado_evaluacion.nombre', 'asc')->get();
            return ResultadoEvaluacionCollection::make($query);
        } else if (count($proceso) > 0) {
            $query = $query->select('mas_resultado_evaluacion.id', 'mas_resultado_evaluacion.nombre', 'mas_resultado_evaluacion.estado')->where('id', '=', Constants::RESULTADO_EVALUACION['comisorio_eje'])->where('estado', Constants::ESTADOS['activo'])->get();
            return ResultadoEvaluacionCollection::make($query);
        }

        $query = $query->select('mas_resultado_evaluacion.id', 'mas_resultado_evaluacion.nombre', 'mas_resultado_evaluacion.estado')->where('estado', Constants::ESTADOS['activo'])->orderBy('mas_resultado_evaluacion.nombre', 'asc')->get();
        return ResultadoEvaluacionCollection::make($query);
    }
    ...
    public function getNombreEvaluacion($id_evaluacion)
    {
        ...
        $query = ResultadoEvaluacionModel::query();
        $query = $query->where("id", $id_evaluacion)->select('mas_resultado_evaluacion.id', 'mas_resultado_evaluacion.nombre', 'mas_resultado_evaluacion.estado')->get();
