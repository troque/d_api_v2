...
{
    public function repartoAleatorioParametrizado($id_proceso_disciplinario, $id_etapa, $id_fase, $descripcion, $id_rol, $busqueda_usuario, $modulo, $reparto_habilitado, $pruebas = 0)
    {
        try {
            $inner_modulo = null;
            $select_modulo = null;
            ...
            if ($busqueda_usuario && count($respuesta_usuario_actual) <= 0) { //SI NO SE BUSCA USUARIO, SE BUSCARA UN REMPLAZO
                $resultado = $this->repartoAleatorioParametrizado($id_proceso_disciplinario, $id_etapa, $id_fase, null, $id_rol, null, $modulo, true, $pruebas + 1);
                return $resultado;
